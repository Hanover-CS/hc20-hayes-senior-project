...
describe("Creating Default Playable Characters", function(){
...
function experiance_Incrementor(char) {
    let accumulator = 0;
    for (let i = 0; i < 20; i++) {
        accumulator += i * 5;
        char.add_Experiance(i * 5);
        assert(char.get_Experiance() === accumulator);
    }
}
...
function level_Incrementer(char) {
    ...
    let previous_Inteligence    = char.get_All_Attributes().inteligence
    ...
    for (let i = 0; i < 10; i++) {
        ...
        assert(char.get_All_Attributes().inteligence    == previous_Inteligence + guide.inteligence * (i+1));
